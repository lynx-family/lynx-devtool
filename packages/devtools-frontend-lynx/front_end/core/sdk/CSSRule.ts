...
export class CSSRule {
  ...
  _getStyleSheetHeader(styleSheetId: string): CSSStyleSheetHeader {
    ...
    if (!styleSheetHeader) {
      envLogger.error(`StyleSheet of Id ${styleSheetId} not found.`, {
        tag: 'Styles'
      });
      reportToStatistics('devtool_css_model', {
        type: '_getStyleSheetHeader',
        pageType: 'lynx',
        eventType: 'stylesheet_not_found'
      });
    }
...
export class CSSStyleRule extends CSSRule {
  ...
  lineNumberInSource(selectorIndex: number): number {
    ...
    const styleSheetHeader = this._getStyleSheetHeader(this.styleSheetId);
    return styleSheetHeader?.lineNumberInSource(selector.range.startLine) ?? 0;
  }
  ...
  columnNumberInSource(selectorIndex: number): number|undefined {
    ...
    const styleSheetHeader = this._getStyleSheetHeader(this.styleSheetId);
    return styleSheetHeader?.columnNumberInSource(selector.range.startLine, selector.range.startColumn) ?? undefined;
  }
