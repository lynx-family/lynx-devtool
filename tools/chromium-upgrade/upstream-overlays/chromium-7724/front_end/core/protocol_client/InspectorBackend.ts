...
type PluginBridgeGlobal = typeof globalThis&{
  postPluginMessage?: (name: string) => (topic: string, msg: string|Record<string, unknown>) => void;
  addPluginEventListener?: (name: string) => (topic: string, listener: (msg: unknown) => void) => void;
};

export const postPluginMessage = (name: string, msg: string|Record<string, unknown>): void => {
  (globalThis as PluginBridgeGlobal).postPluginMessage?.(name)(`Extensions.${name}`, msg);
};

export const addPluginEventListener = (name: string, listener: (msg: unknown) => void): void => {
  (globalThis as PluginBridgeGlobal).addPluginEventListener?.(name)(`Extensions.${name}`, listener);
};
...
class EnvLogger {
  debug(msg: Record<string, unknown>|string, categories: EnvLogCategoryObject): void {
    this.log(LevelType.Debug, msg, categories);
  }

  info(msg: Record<string, unknown>|string, categories: EnvLogCategoryObject): void {
    this.log(LevelType.Info, msg, categories);
  }

  warn(msg: Record<string, unknown>|string, categories: EnvLogCategoryObject): void {
    this.log(LevelType.Warn, msg, categories);
  }

  error(msg: Record<string, unknown>|string, categories: EnvLogCategoryObject): void {
    this.log(LevelType.Error, msg, categories);
  }

  log(level: LevelType, msg: Record<string, unknown>|string, categories: EnvLogCategoryObject): void {
    const logObject = this.assembleEnvLogObject(level, msg, categories);
  ...
  private assembleEnvLogObject(
      level: LevelType, msg: Record<string, unknown>|string, categories: EnvLogCategoryObject): EnvLogObject {
