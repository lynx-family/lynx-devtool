...
const IFRAME_SESSION_ID = Number.parseInt(new URLSearchParams(globalThis.location?.search ?? '').get('sessionId') ?? '0', 10);
...
export const reportToStatistics = (
    name: string, categories?: Record<string, unknown>, metrics?: Record<string, unknown>): void => {
  sendWindowMessage({
    type: 'statistics',
    content: [
      'sendEvent',
